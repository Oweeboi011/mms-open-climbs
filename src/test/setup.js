...
vi.mock("firebase/auth", () => ({
  ...
  createUserWithEmailAndPassword: vi.fn(() =>
    Promise.resolve({ user: { uid: "u1", displayName: "", email: "" } }),
  ),
...
beforeEach(() => {
  console.error = (...a) => {
    if (!SUPPRESS_PATTERNS.some((p) => String(a[0]).match(p))) _err(...a);
  };
  console.warn = (...a) => {
    if (!SUPPRESS_PATTERNS.some((p) => String(a[0]).match(p))) _warn(...a);
  };
});

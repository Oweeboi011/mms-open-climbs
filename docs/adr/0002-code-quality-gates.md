...
- ~~Work the 19 react-hooks warnings down toward 0, lowering `--max-warnings` each time.~~ Done: all 20 (19 baseline + 1 introduced later) fixed, `--max-warnings` lowered to 0. Two effects that generically call setState — a cross-component signal and a mount-time data fetch — kept their `useEffect` with a narrow, commented `eslint-disable-next-line`, since both are legitimate uses the rule's heuristic doesn't distinguish from derived-state anti-patterns.
- Functions branch coverage had quietly drifted under its own 64% floor (to 60.68%) while `npm run lint` was failing first and masking it. Added `functions/tests/releaseNotes.test.js` covering `sendReleaseNoteEmail`, `getReleaseNoteCommitOptions`, and `generateReleaseNoteDraft` — previously untested callables. Coverage is now 83/83/74.19/68.22 (lines/stmts/fns/branches); floors raised to 83/83/74/68 to match.
- `jscpd` duplication had also drifted over its own 4% ceiling (to 4.45%) for the same masked-by-lint reason, driven mostly by the admin document-management feature copy-pasting the entire member-facing `DocumentPrompt` (`MyRegistrations.jsx`) into `AdminDocumentModal.jsx`. Extracted the shared modal chrome/form into `src/components/DocumentUploadModal.jsx`, parameterized for the copy/validation/save differences between the two flows. Duplication is now 3.71%. The remaining largest cluster — a registrant-table section shared across `AllRegistrations.jsx`/`ClimbDetail.jsx`/`ManagePayments.jsx` (~200 lines) — is real but wasn't pulled in scope here; still tracked as follow-up.

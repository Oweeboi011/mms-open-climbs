...
| Waitlist promotion | Any trigger that frees a seat (`onRegistrationUpdated`: pending/confirmed → anything else; `onRegistrationDeleted` of a seat-holder; `onClimbUpdated` raising `maxParticipants`) | `promoteFromWaitlist` moves the oldest waitlisted registrations (by `createdAt`) to `pending` with `promotedFromWaitlistAt`, emails and notifies the member, emails officers (CC admins). Skipped when `climb.waitlistAutoPromote === false`, or the climb is cancelled, postponed or over |
